document.addEventListener("DOMContentLoaded", () => {
  ...
  const input_st_adress = input_form?.querySelector("#st_adress");
  ...
  const onSaveHandler = () => {
    ...
    const text_st_adress = input_st_adress.value;
    ...
    //insNan(변수) 변수에 포함된 값에 문자열이 섞여있나?true;
    if (isNaN(text_st_grade)) {
      alert("학년은 반드시 숫자로 입력해야 합니다");
      input_st_grade.select();
      return;
    }
    const input_st_grade = Number(text_st_grade);
    if (input_st_grade < 1 || input_st_grade > 4) {
      alert("학는은 1~4학년까지만 입력해야 합니다");
      input_st_grade.select();
      return;
    }
    if (!onValidate(text_st_tel, "전화번호", input_st_tel)) return false;

    if (!onValidate(text_st_adress, "주소", input_st_adress)) return false;
